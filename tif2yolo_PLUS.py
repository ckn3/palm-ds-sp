...
def make_it_white(idx, csv_2_path, tif_path):
    ...
    # 打印调试信息
    # print("white area:", sub_x2 - sub_x1, sub_y2 -sub_y1)
...
def check_bbox_is_valid(csv_2_path, tif_path, output_dir, bbox_pixel_length=113):
    ...
    for index, row in df.iterrows():
        ...
        for index1 in true_bboxs:
            # print(index)
            temp = 0
            col_labels = ['Especie', 'x_center', 'y_center', 'X_L', 'Y_L', 'X_R', 'Y_R','x1','y1']
            classes, sub_center_x, sub_center_y, sub_xl, sub_yl, sub_xr, sub_yr, x1, y1 = df.loc[index1, col_labels]
            # print(sub_center_x, sub_center_y)

            #保存txt形式的yolo的label
            if index1 not in used:
                sub_center_x -= (center_x - img_size / 2)
                sub_center_y -= (center_y - img_size / 2)
                sub_xl = (sub_xl - center_x - img_size / 2) / 2
                sub_yl = (sub_yl - center_y - img_size / 2) / 2
                sub_xr = (sub_xr - center_x - img_size / 2) / 2
                sub_yr = (sub_yr - center_y - img_size / 2) / 2
                print("bbox size:", sub_xr - sub_xl, sub_yr - sub_yl)

                yolo_x_center = sub_center_x / img_size
                yolo_y_center = sub_center_y / img_size
                yolo_width = (sub_xr - sub_xl) / img_size
                yolo_height = (sub_yr - sub_yl) / img_size
                # print(yolo_x_center, yolo_y_center, yolo_width, yolo_height)
        ...
        for index1 in true_bboxs:
            ...
            for index2 in false_bboxs:
                ...
                x_min, y_min, x_max, y_max = df.loc[index2, col_labels]
                # 检查是否重叠:temp = 0时是一直没有进else，所以没有重合
                if sub_xr < x_min or sub_xl > x_max or sub_yr < y_min or sub_yl > y_max:
                    temp = temp
                else:
                    temp += 1
                if temp == 0:
...
#%%
def delet_empty_img(output_dir):
    import os
    import glob
    ...
    # 提取基础的文件名（不含扩展名）
    png_base_names = [os.path.splitext(os.path.basename(png_file))[0] for png_file in png_files]
    txt_base_names = [os.path.splitext(os.path.basename(txt_file))[0] for txt_file in txt_files]

    # 创建一个集合以便快速查找
    txt_base_names_set = set(txt_base_names)

    # 找到缺失的TXT文件对应的PNG文件，并删除这些PNG文件
    for png_base_name in png_base_names:
        if png_base_name not in txt_base_names_set:
            # 获取PNG文件的完整路径
            png_file_to_delete = os.path.join(folder_path, png_base_name + '.png')
            # 删除PNG文件
            os.remove(png_file_to_delete)
            print(f'Deleted: {png_file_to_delete}')
...
delet_empty_img(output_dir)#在原图中涂白区域后，会导致有些图里没有palm，所以把这类图片删掉
